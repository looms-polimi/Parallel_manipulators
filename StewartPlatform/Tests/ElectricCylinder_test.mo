...
model ElectricCylinder_test
  ...
  StewartPlatform.Components.ElectricCylinder electricCylinder(pj_initialVelFixed = true)  annotation(
  ...
  Modelica.Mechanics.Rotational.Components.IdealGear idealGear(ratio = 1 / electricCylinder.ECparameters.ratio) annotation(
    Placement(visible = true, transformation(origin = {-58, 34}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Sources.Sine sine(amplitude = ECparameters.workingStroke / 2,f = 1, offset = ECparameters.minLength + ECparameters.workingStroke / 2) annotation(
