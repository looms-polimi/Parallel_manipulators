...
model ServoMotor "Permanent magnet synchronous induction machine"
 ...
 // Imports
      import Modelica.Math.Polynomials.roots;
  ...
  Interfaces.AxisControlBus axisControlBus annotation(
    Placement(transformation(extent = {{-70, -10}, {-50, 10}}), iconTransformation(extent = {{-24, 24}, {24, -24}}, rotation = 90, origin = {-58, 4})));

  parameter SI.Frequency bandwidth = SMparameters.bandwidth "Bandwidth of the internal torque control" annotation(
    HideResult = true);
  parameter Integer np = SMparameters.np "Number of pole pairs (Integer)" annotation(
    HideResult = true);
  parameter SI.Resistance Rs = SMparameters.Rs "Stator resistance per phase" annotation(
    HideResult = true);
  parameter SI.Inductance L = SMparameters.L "Stator inductance per phase" annotation(
    HideResult = true);
  parameter SI.ElectricalTorqueConstant Psi = SMparameters.Psi "Torque constant, T=np*Psi*i_sq" annotation(
    HideResult = true);
  parameter Real Ke(unit = "V/(rad/s)", min = 0) = SMparameters.Ke "Voltage constant, the induced motor EMF is E=Ke*omega, omega = electrical_speed" annotation(
    HideResult = true);
  parameter SI.Voltage Vmax = SMparameters.Vmax "Inverter max voltage" annotation(
    HideResult = true);
  parameter SI.Torque Tmax = SMparameters.Tmax "Max torque" annotation(
    HideResult = true);
  parameter SI.Current Imax = SMparameters.Imax "Inverter max current" annotation(
    HideResult = true);
  parameter SI.Inductance Ls = SMparameters.Ls "Synchronous inductance" annotation(
    HideResult = true);
  // Variables
protected
  Modelica.Blocks.Nonlinear.Limiter limiter(uMax = Tmax, uMin = -Tmax) annotation(
  ...
  Modelica.Blocks.Continuous.FirstOrder controlBandwidth(T = 1 / bandwidth) annotation(
...
//Saturation of input torqueRef within [Tmax, -Tmax] -> signal Tref
  is_q0 = Tref / np / Psi;
//Computation of the current component required to reproduce Tref
  (is_d, is_q) = StewartPlatform.Functions.servoMotorCurrents(Rs, omega, Ls, Ke, Vmax, Imax, is_q0);
//Computation of the current components is_q and is_d that can be applied with respect to the current and voltage limits
  ...
  Pmecc = w * T;
  Pelect = Vs_d * is_d + Vs_q * is_q;
//Real(Vs*conj(is)) with Vs and is space phasor vectors
  C_omega_b = roots({Ls ^ 2 * is_q ^ 2 + Ke ^ 2, 2 * Rs * Ke * is_q, Rs ^ 2 * is_q ^ 2 - Vmax ^ 2});
  ...
  connect(angleSensor.flange, flange) annotation(
    Line(points = {{80, -40}, {100, -40}, {100, 0}}, color = {0, 0, 0}));
  connect(speedSensor.flange, flange) annotation(
    Line(points = {{80, 40}, {100, 40}, {100, 0}}, color = {0, 0, 0}));
  connect(limiter.y, controlBandwidth.u) annotation(
    Line(points = {{-19, 0}, {-12, 0}}, color = {0, 0, 127}));
  connect(axisControlBus.refTorque, limiter.u) annotation(
    Line(points = {{-59.95, 0.05}, {-50, 0.05}, {-50, 0}, {-42, 0}}, color = {255, 204, 51}, thickness = 0.5),
    Text(string = "%first", index = -1, extent = {{-6, 3}, {-6, 3}}));
  connect(angleSensor.phi, axisControlBus.encoder) annotation(
    Line(points = {{59, -40}, {-59.95, -40}, {-59.95, 0.05}}, color = {0, 0, 127}),
