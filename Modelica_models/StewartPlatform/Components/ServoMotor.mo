...
// Parameters
  parameter StewartPlatform.Types.ServoMotorParameters SMparameters "Parameters of the servo motor";
  parameter Boolean sm_enableLimiter = true "=true, if you want to limit the input reference torque within interval [-SMparameters.Tmax, SMparameters.Tmax]"; annotation (choices(checkBox=true));
...
protected
  Modelica.Blocks.Nonlinear.Limiter limiter(uMax=if sm_enableLimiter then SMparameters.Tmax else inf, uMin=if sm_enableLimiter then -SMparameters.Tmax else -inf) annotation(Placement(transformation(extent = {{-40, -10}, {-20, 10}})));
